...
describe('Vault — User Profile', () => {
  ...
  test('save + load persists normalized answers', async () => {
    initDatabase(':memory:');
  ...
  test('clear removes saved profile', async () => {
    ...
    await saveUserProfile({ preferred_name: 'Alex' });
    await clearUserProfile();

    expect(await getUserProfile()).toBeNull();
    expect(await findEntities({ name: 'Alex' })).toHaveLength(0);
    expect(await findFacts({ predicate: 'preferred_name' })).toHaveLength(0);
  });

  test('save syncs profile answers into the vault knowledge base', async () => {
    initDatabase(':memory:');
    ...
    const entities = await findEntities({ name: 'Alex' });
  ...
  test('save derives alias facts from free-form profile answers', async () => {
    initDatabase(':memory:');
    ...
    const entities = await findEntities({ name: 'Sebastian' });
    expect(entities).toHaveLength(1);
  ...
  test('prompt formatter includes answered fields only', async () => {
    initDatabase(':memory:');
  ...
  test('prompt formatter indents multiline answers to keep them contained', async () => {
    initDatabase(':memory:');
