...
export async function extractAndStore(
  ...
  try {
    ...
    for (const entityData of extraction.entities) {
      ...
      const existing = await findEntities({ name, type });
    ...
    for (const factData of extraction.facts) {
      ...
      await createFact(subjectId, predicate, object, {
 ...
 */
export async function extractGoalCompletion(goal: {
  id: string;
  title: string;
  level: string;
  score: number;
  ...
  try {
    ...
    const existing = await findEntities({ name: goal.title, type: 'concept' });
    let entityId: string;

    if (existing.length > 0) {
      entityId = existing[0]!.id;
    } else {
      const entity = await createEntity('concept', goal.title, {
        goal_id: goal.id,
    ...
    await createFact(entityId, 'goal_final_score', goal.score.toFixed(2), {
      confidence: 1.0,
      source: 'goal_completion',
    });

    await createFact(entityId, 'goal_outcome', goal.status, {
      confidence: 1.0,
      source: 'goal_completion',
    });

    await createFact(entityId, 'goal_level', goal.level, {
      confidence: 1.0,
      source: 'goal_completion',
    });

    if (goal.estimated_hours !== null) {
      await createFact(entityId, 'estimated_hours', goal.estimated_hours.toString(), {
        confidence: 1.0,
        source: 'goal_completion',
      });
    }
    ...
    if (goal.actual_hours > 0) {
      await createFact(entityId, 'actual_hours', goal.actual_hours.toFixed(1), {
        confidence: 1.0,
        source: 'goal_completion',
      });
    }
    ...
    if (goal.completed_at) {
      ...
      await createFact(entityId, 'days_to_complete', durationDays.toString(), {
        confidence: 1.0,
        source: 'goal_completion',
      });
    }
    ...
    if (goal.estimated_hours !== null && goal.actual_hours > 0) {
      ...
      await createFact(entityId, 'estimation_accuracy', accuracy, {
        confidence: 1.0,
        source: 'goal_completion',
      });
    }
    ...
    if (goal.tags.length > 0) {
      await createFact(entityId, 'goal_tags', goal.tags.join(', '), {
        confidence: 1.0,
        source: 'goal_completion',
      });
    }
