...
export class DeferredExecutor {
  ...
  async executeApproved(requestId: string): Promise<string> {
    ...
    try {
      ...
      // Mark as executed
      await this.approvalManager.markExecuted(requestId, result.slice(0, 2000));
      ...
      await this.approvalManager.markExecuted(requestId, errorStr);
   ...
   */
  async recordDenial(request: ApprovalRequest): Promise<void> {
    if (this.learner) {
      await this.learner.recordDecision(
        request.action_category as ActionCategory,
        request.tool_name,
