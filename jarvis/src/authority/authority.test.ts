...
describe('ApprovalManager', () => {
  ...
  test('creates and retrieves approval request', async () => {
    ...
    const retrieved = await mgr.getRequest(req.id);
  ...
  test('approve changes status', async () => {
    ...
    const approved = await mgr.approve(req.id, 'dashboard');
    expect(approved).not.toBeNull();
    expect(approved!.status).toBe('approved');
    expect(approved!.decided_by).toBe('dashboard');
  });

  test('deny changes status', async () => {
    const mgr = new ApprovalManager();
    const req = await mgr.createRequest({
      agentId: 'a1', agentName: 'PA', toolName: 'send_email',
      toolArguments: {}, actionCategory: 'send_email',
    ...
    const denied = await mgr.deny(req.id, 'telegram');
    expect(denied).not.toBeNull();
    expect(denied!.status).toBe('denied');
  });

  test('cannot approve already decided request', async () => {
    const mgr = new ApprovalManager();
    const req = await mgr.createRequest({
      agentId: 'a1', agentName: 'PA', toolName: 'send_email',
      toolArguments: {}, actionCategory: 'send_email',
    ...
    await mgr.deny(req.id, 'dashboard');
    const result = await mgr.approve(req.id, 'dashboard');
    expect(result).toBeNull();
  });

  test('getPending returns only pending', async () => {
    const mgr = new ApprovalManager();
    await mgr.createRequest({
      agentId: 'a1', agentName: 'PA', toolName: 'send_email',
      toolArguments: {}, actionCategory: 'send_email',
      urgency: 'normal', reason: 'test', context: '',
    });
    const req2 = await mgr.createRequest({
    ...
    await mgr.approve(req2.id, 'dashboard');
  ...
  test('findByShortId works', async () => {
    const mgr = new ApprovalManager();
    const req = await mgr.createRequest({
      agentId: 'a1', agentName: 'PA', toolName: 'send_email',
      toolArguments: {}, actionCategory: 'send_email',
    ...
    const found = await mgr.findByShortId(req.id.slice(0, 8));
    expect(found).not.toBeNull();
    expect(found!.id).toBe(req.id);
  });

  test('markExecuted updates fields', async () => {
    const mgr = new ApprovalManager();
    const req = await mgr.createRequest({
      agentId: 'a1', agentName: 'PA', toolName: 'send_email',
      toolArguments: {}, actionCategory: 'send_email',
    ...
    await mgr.approve(req.id, 'dashboard');
    await mgr.markExecuted(req.id, 'Email sent successfully');

    const updated = await mgr.getRequest(req.id);
    expect(updated!.status).toBe('executed');
    expect(updated!.execution_result).toBe('Email sent successfully');
  });

  test('expireOld expires old pending requests', async () => {
    const mgr = new ApprovalManager();
    await mgr.createRequest({
      agentId: 'a1', agentName: 'PA', toolName: 'send_email',
      toolArguments: {}, actionCategory: 'send_email',
    ...
    const noneExpired = await mgr.expireOld(999999999);
    expect(noneExpired).toBe(0);

    // Manually update created_at to the past so expiry works
    // Use manual update on DB because expireOld uses DB directly
    await mgr['db'].from('approval_requests')
      .update({ created_at: new Date(Date.now() - 10000).toISOString() })
      .eq('status', 'pending');
      
    const expired = await mgr.expireOld(5000);
...
describe('AuditTrail', () => {
  ...
  test('logs and queries audit entries', async () => {
    ...
    const browserOnly = await trail.query({ action: 'access_browser' });
  ...
  test('getStats aggregates correctly', async () => {
    ...
    await trail.log({ agent_id: 'a1', agent_name: 'PA', tool_name: 't1', action_category: 'read_data', authority_decision: 'allowed', executed: true });
    await trail.log({ agent_id: 'a1', agent_name: 'PA', tool_name: 't2', action_category: 'write_data', authority_decision: 'allowed', executed: true });
    await trail.log({ agent_id: 'a1', agent_name: 'PA', tool_name: 't3', action_category: 'send_email', authority_decision: 'denied', executed: false });

    const stats = await trail.getStats();
...
describe('AuthorityLearner', () => {
  ...
  test('no suggestions below threshold', async () => {
    const learner = new AuthorityLearner(3);
    await learner.recordDecision('send_email', 'send_email', true);
    await learner.recordDecision('send_email', 'send_email', true);

    expect((await learner.getSuggestions()).length).toBe(0);
  });

  test('suggests after threshold consecutive approvals', async () => {
    const learner = new AuthorityLearner(3);
    await learner.recordDecision('send_email', 'send_email', true);
    await learner.recordDecision('send_email', 'send_email', true);
    await learner.recordDecision('send_email', 'send_email', true);

    const suggestions = await learner.getSuggestions();
    expect(suggestions.length).toBe(1);
    expect(suggestions[0]!.actionCategory).toBe('send_email');
    expect(suggestions[0]!.consecutiveApprovals).toBe(3);
  });

  test('denial resets consecutive count', async () => {
    const learner = new AuthorityLearner(3);
    await learner.recordDecision('send_email', 'send_email', true);
    await learner.recordDecision('send_email', 'send_email', true);
    await learner.recordDecision('send_email', 'send_email', false); // reset
    await learner.recordDecision('send_email', 'send_email', true);

    expect((await learner.getSuggestions()).length).toBe(0);
  });

  test('markSuggestionSent prevents re-suggestion', async () => {
    const learner = new AuthorityLearner(2);
    await learner.recordDecision('send_email', 'send_email', true);
    await learner.recordDecision('send_email', 'send_email', true);

    expect((await learner.getSuggestions()).length).toBe(1);

    await learner.markSuggestionSent('send_email', 'send_email');
    expect((await learner.getSuggestions()).length).toBe(0);
  });
});
