...
export async function recordUserProfileTurn(text: string): Promise<FollowupTurnResult> {
...
export async function maybeCreateUserProfileFollowupPrompt(): Promise<string | null> {
...
export async function clearUserProfileFollowupState(): Promise<void> {
  await deleteSetting(USER_PROFILE_FOLLOWUP_STATE_KEY);
}

async function getFollowupState(): Promise<UserProfileFollowupState> {
  const raw = await getSetting(USER_PROFILE_FOLLOWUP_STATE_KEY);
...
async function saveFollowupState(state: UserProfileFollowupState): Promise<void> {
  await setSetting(USER_PROFILE_FOLLOWUP_STATE_KEY, JSON.stringify(state));
}
