...
export class AgentService implements Service, IAgentService {
  ...
  private async buildFullSystemPrompt(channel: string, userMessage?: string): Promise<string> {
    ...
    const personality = this.personality ?? await loadPersonality();
  ...
  private async buildPromptContext(userMessage?: string): Promise<PromptContext> {
    ...
    try {
      ...
      if (mgr) {
        const list = await mgr.listSidecars();
        hasSidecars = list.length > 0;
      }
    } catch { /* ignore */ }
    ...
    // Parallel fetch tasks for efficiency
    ...
    if (userMessage) {
      ...
      tasks.push((async () => {
        try {
          const webappInstructions = await getWebappInstructionsForMessage(userMessage);
    ...
    tasks.push((async () => {
      try {
        const { getActiveGoalsSummary } = await import('../vault/retrieval.ts');
    ...
    // Authority rules (synchronous description lookup)
  ...
  private async learnFromInteraction(
    ...
    let personality = this.personality ?? await loadPersonality();
