...
export class BackgroundAgentService implements Service, IAgentService {
  ...
  async handleMessage(text: string, channel: string = 'system'): Promise<string> {
    ...
    this.busy = true;
    try {
      const systemPrompt = await this.buildSystemPrompt(channel);
  ...
  private async buildSystemPrompt(channel: string): Promise<string> {
    if (!this.role) return '';
   ...
   */
  private async getRecentChatContext(messageCount: number = 20): Promise<{
    ...
    try {
      const recent = await getRecentConversation('websocket');
      if (!recent) return { transcript: null, lastUserMessageAt: null, lastAssistantMessageAt: null, minutesSinceLastUserMessage: null };

      const messages = await getMessages(recent.conversation.id, { limit: messageCount });
  ...
  private async buildHeartbeatPrompt(coalescedEvents?: string): Promise<string> {
    ...
    // --- RECENT CHAT CONTEXT ---
    const chat = await this.getRecentChatContext(20);
    if (chat.transcript) {
    ...
    // --- ACTIVE GOALS ---
    try {
      const goalsSummary = await getActiveGoalsSummary();
      if (goalsSummary) {
  ...
  private async buildPromptContext(): Promise<PromptContext> {
    ...
    // Get due commitments
    try {
      const due = await getDueCommitments();
      const upcoming = await getUpcoming(5);
    ...
    try {
      const activeContent = (await findContent({})).filter(
        (c) => c.stage !== 'published'
      ).slice(0, 10);
    ...
    try {
      const observations = await getRecentObservations(undefined, 10);
