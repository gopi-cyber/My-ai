...
export class HealthMonitor {
   ...
   */
  private async getDatabaseStats(): Promise<{ connected: boolean; size: number }> {
    let connected = false;
    let size = 0;

    try {
      const db = getDb();
      if (!db) return { connected: false, size: 0 };

      // Simple query to check if DB is responsive.
      // Use 'commitments' as a canonical table to check connectivity.
      const { error } = await db.from('commitments').select('id').limit(1);
      connected = !error;

      // Get DB file size if it's not in-memory (only relevant for SQLite-backed mocks/configurations)
   ...
   */
  async formatHealth(): Promise<string> {
    const health = await this.getHealth();
    const lines: string[] = [];
