...
export class ChannelService implements Service {
  ...
  private async handleChannelMessage(msg: ChannelMessage): Promise<string> {
    ...
    const conversation = await getOrCreateConversation(channelTag);
    await addMessage(conversation.id, { role: 'user', content: msg.text });
    ...
    await addMessage(conversation.id, { role: 'assistant', content: response });
