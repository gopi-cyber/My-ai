...
export class CommitmentExecutor {
  ...
  start(): void {
    ...
    this.checkTimer = setInterval(() => {
      this.checkAndAnnounce().catch((err) =>
    ...
    this.checkAndAnnounce().catch((err) =>
      console.error('[Executor] Initial check error:', err)
    );
  ...
  async checkAndAnnounce(): Promise<void> {
    try {
      const now = Date.now();
      const dueNow = await getDueCommitments(); // when_due <= now
      const upcoming = await getUpcoming(20); // all upcoming with when_due
    ...
    for (const [id, state] of this.pending) {
      ...
      try {
        ...
        try {
          ...
          await updateCommitmentStatus(state.commitmentId, 'failed', reason);
        } catch { /* ignore */ }
      }
    }
  }
  ...
  private async executeCommitment(state: ExecutionState): Promise<void> {
    ...
    // Mark as active
    try {
      await updateCommitmentStatus(state.commitmentId, 'active');
    } catch { /* ignore */ }
    ...
    try {
      await updateCommitmentStatus(state.commitmentId, 'completed', resultSummary);
    } catch (err) {
