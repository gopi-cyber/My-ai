...
export function createApiRoutes(ctx: ApiContext): Record<string, unknown> {
  return {
    // --- Health ---
    '/api/health': {
      GET: async () => json(await ctx.healthMonitor.getHealth()),
    },
    ...
    '/api/vault/entities': {
      GET: async (req: Request) => {
        ...
        return json(await findEntities(query));
      },
    },
    ...
    '/api/vault/entities/:id/facts': {
      GET: async (req: Request & { params: { id: string } }) => {
        return json(await findFacts({ subject_id: req.params.id }));
      },
    },

    '/api/vault/entities/:id/relationships': {
      GET: async (req: Request & { params: { id: string } }) => {
        return json(await getEntityRelationships(req.params.id));
      },
    },
    ...
    '/api/vault/facts': {
      GET: async (req: Request) => {
        ...
        return json(await findFacts(query));
      },
    },
    ...
    '/api/vault/relationships': {
      GET: async (req: Request) => {
        ...
        if (type) query.type = type;
        return json(await findRelationships(query));
      },
    },
    ...
    '/api/vault/search': {
      GET: async (req: Request) => {
        ...
        if (q) {
          ...
          const nameMatches = await searchEntitiesByName(q);
          ...
          const { data: factRows } = await db
            .from('facts')
            .select('subject_id')
            .or(`predicate.ilike.${safeQ},object.ilike.${safeQ}`)
            .limit(200);
          ...
          const allEntities = await findEntities(type ? { type } : {});
        ...
        for (const id of entityIds) {
          ...
          const facts = await findFacts({ subject_id: id });
    ...
    '/api/vault/commitments': {
      GET: async (req: Request) => {
        ...
        if (upcoming) {
          return json(await getUpcoming(parseInt(upcoming) || 10));
        }
        ...
        return json(await findCommitments(query));
      },
      POST: async (req: Request) => {
        try {
          ...
          const commitment = await createCommitment(body.what, {
    ...
    '/api/vault/commitments/reorder': {
      POST: async (req: Request) => {
        try {
          ...
          await reorderCommitments(body.items);
          return json({ ok: true });
        } catch (err) {
          return error('Invalid request body');
        }
    ...
    '/api/vault/commitments/:id': {
      ...
      PATCH: async (req: Request & { params: { id: string } }) => {
        try {
          ...
          const updated = await updateCommitmentStatus(id, body.status, body.result);
    ...
    '/api/vault/conversations': {
      GET: async (req: Request) => {
        ...
        const db = getDb();
        let query = db.from('conversations').select('*').order('last_message_at', { ascending: false }).limit(limit);
      ...
      GET: async (req: Request) => {
        ...
        if (channel === 'all') {
          ...
          for (const ch of channels) {
            const result = await getRecentConversation(ch);
            if (result) results[ch] = result;
          }
          return json(results);
        }

        const result = await getRecentConversation(channel);
    ...
    '/api/vault/observations': {
      GET: async (req: Request) => {
        ...
        return json(await getRecentObservations(type, limit));
      },
    },
    ...
    '/api/calendar': {
      GET: async (req: Request) => {
        ...
        const { data: dueRows } = await db
          ...
          .gte('when_due', rangeStart)
          .lt('when_due', rangeEnd);
    ...
    // --- Personality ---
    '/api/personality': {
      GET: async () => json(await getPersonality()),
    },
    ...
    '/api/user-profile': {
      ...
      POST: async (req: Request) => {
        try {
          ...
          const profile = await saveUserProfile(body.answers ?? {});
          return json({
            ok: true,
            profile,
            answered_count: countAnsweredUserProfileQuestions(profile),
    ...
    '/api/content': {
      GET: async (req: Request) => {
        ...
        return json(await findContent(query));
      },
      POST: async (req: Request) => {
        try {
          const body = await req.json() as {
          ...
          const item = await createContent(body.title, {
            body: body.body,
    ...
    '/api/content/:id': {
      ...
      PATCH: async (req: Request & { params: { id: string } }) => {
        try {
          ...
          const updated = await updateContent(req.params.id, body);
    ...
    '/api/content/:id/advance': {
      POST: async (req: Request & { params: { id: string } }) => {
        const updated = await advanceStage(req.params.id);
    ...
    '/api/content/:id/regress': {
      POST: async (req: Request & { params: { id: string } }) => {
        const updated = await regressStage(req.params.id);
    ...
    '/api/content/:id/notes': {
      GET: async (req: Request & { params: { id: string } }) => {
        const params = getSearchParams(req);
        const stage = params.get('stage') as ContentStage | null;
        return json(await getStageNotes(req.params.id, stage ?? undefined));
      },
      POST: async (req: Request & { params: { id: string } }) => {
        try {
          const body = await req.json() as {
          ...
          const note = await addStageNote(req.params.id, body.stage, body.note, body.author);
    ...
    '/api/content/:id/attachments': {
      GET: async (req: Request & { params: { id: string } }) => {
        return json(await getAttachments(req.params.id));
      },
      POST: async (req: Request & { params: { id: string } }) => {
        try {
    ...
    '/api/content/:id/attachments/:aid': {
      DELETE: async (req: Request & { params: { id: string; aid: string } }) => {
        // Verify attachment belongs to this content item before deleting
        const attachment = await getAttachment(req.params.aid);
        if (!attachment || attachment.content_id !== req.params.id) {
          return error('Attachment not found', 404);
        }
        const deleted = await deleteAttachment(req.params.aid);
    ...
    '/api/authority/status': {
      GET: async () => {
        ...
        return json({
          ...
          pending_approvals: (await approvals?.getPending())?.length ?? 0,
    ...
    '/api/authority/approvals': {
      GET: async (req: Request) => {
        ...
        if (status === 'pending') {
          return json(await ctx.approvalManager.getPending());
        }
        return json(await ctx.approvalManager.getHistory({
    ...
    '/api/authority/approvals/:id/approve': {
      POST: async (req: Request & { params: { id: string } }) => {
        ...
        const approved = await ctx.approvalManager.approve(requestId, 'dashboard');
        ...
        const updated = await ctx.approvalManager.getRequest(requestId);
    ...
    '/api/authority/approvals/:id/deny': {
      POST: async (req: Request & { params: { id: string } }) => {
        ...
        const requestId = req.params.id;
        const denied = await ctx.approvalManager.deny(requestId, 'dashboard');
        if (!denied) return error('Request not found or already decided', 404);

        // Record denial for learning
        await ctx.deferredExecutor.recordDenial(denied);
    ...
    '/api/authority/audit': {
      GET: async (req: Request) => {
        ...
        const params = getSearchParams(req);
        return json(await ctx.auditTrail.query({
          agentId: params.get('agent_id') || undefined,
          action: (params.get('action') as ActionCategory) || undefined,
    ...
    '/api/authority/audit/stats': {
      GET: async (req: Request) => {
        ...
        return json(await ctx.auditTrail.getStats(since));
      },
    },
    ...
    '/api/authority/learning/suggestions': {
      GET: async () => {
        ...
        return json(await ctx.learner.getSuggestions());
      },
    },
    ...
    '/api/authority/learning/accept': {
      POST: async (req: Request) => {
        ...
        try {
          ...
          await ctx.learner.markSuggestionSent(body.action, body.tool_name ?? '');
    ...
    '/api/authority/learning/dismiss': {
      POST: async (req: Request) => {
        ...
        try {
          const body = await req.json() as { action: ActionCategory; tool_name: string };
          if (!body.action) return error('Missing "action" field');
          await ctx.learner.resetPattern(body.action, body.tool_name ?? '');
          return json({ ok: true });
        } catch (err) {
          return error('Invalid request body');
        }
    ...
    '/api/workflows': {
      GET: async (req: Request) => {
        try {
          const { findWorkflows } = await import('../vault/workflows.ts');
          const params = getSearchParams(req);
          const query: any = {};
          ...
          return json(await findWorkflows(query));
        } catch (err) { return error(`${err}`); }
      },
      POST: async (req: Request) => {
        try {
          const { createWorkflow, createVersion } = await import('../vault/workflows.ts');
          const body = await req.json() as any;
          if (!body.name) return error('name is required');
          const wf = await createWorkflow(body.name, {
            description: body.description,
            authority_level: body.authority_level,
            tags: body.tags,
          });
          if (body.definition) {
            await createVersion(wf.id, body.definition, body.changelog ?? 'Initial version');
          }
          return json(wf, 201);
        } catch (err) { return error(`${err}`); }
      },
    ...
    '/api/workflows/import': {
      POST: async (req: Request) => {
        try {
          const { importWorkflowYaml } = await import('../workflows/yaml.ts');
          const { createWorkflow, createVersion, setVariable } = await import('../vault/workflows.ts');
          const yamlText = await req.text();
          const imported = importWorkflowYaml(yamlText);
          const wf = await createWorkflow(imported.name, {
            description: imported.description,
            authority_level: imported.authority_level,
            tags: imported.tags,
          });
          await createVersion(wf.id, imported.definition, 'Imported');
          for (const [k, v] of Object.entries(imported.variables)) {
            await setVariable(wf.id, k, v);
          }
          return json(wf, 201);
    ...
    '/api/workflows/:id': {
      GET: async (req: Request) => {
        try {
          const { getWorkflow } = await import('../vault/workflows.ts');
      ...
      PATCH: async (req: Request) => {
        try {
          const { updateWorkflow } = await import('../vault/workflows.ts');
          const url = new URL(req.url);
          const id = url.pathname.split('/').pop()!;
          const body = await req.json() as any;
      ...
      DELETE: async (req: Request) => {
        try {
          const { deleteWorkflow } = await import('../vault/workflows.ts');
          const url = new URL(req.url);
          const id = url.pathname.split('/').pop()!;
    ...
    '/api/workflows/:id/versions': {
      GET: async (req: Request) => {
        try {
          const { getVersionHistory } = await import('../vault/workflows.ts');
          const url = new URL(req.url);
          const parts = url.pathname.split('/');
          const id = parts[parts.length - 2];
          if (!id) return error('ID required');
          return json(await getVersionHistory(id));
        } catch (err) { return error(`${err}`); }
      },
      POST: async (req: Request) => {
        try {
          const { createVersion } = await import('../vault/workflows.ts');
          const url = new URL(req.url);
          const parts = url.pathname.split('/');
          const id = parts[parts.length - 2];
          if (!id) return error('ID required');
          ...
          const version = await createVersion(id, body.definition, body.changelog);
    ...
    '/api/workflows/:id/executions': {
      GET: async (req: Request) => {
        try {
          const { findExecutions } = await import('../vault/workflows.ts');
          const url = new URL(req.url);
          const parts = url.pathname.split('/');
          const id = parts[parts.length - 2];
          return json(await findExecutions({ workflow_id: id }));
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/workflows/:id/variables': {
      GET: async (req: Request) => {
        try {
          const { getVariables } = await import('../vault/workflows.ts');
          const url = new URL(req.url);
          const parts = url.pathname.split('/');
          const id = parts[parts.length - 2];
          if (!id) return error('ID required');
          return json(await getVariables(id));
        } catch (err) { return error(`${err}`); }
      },
      PATCH: async (req: Request) => {
        try {
          const { setVariable, getVariables } = await import('../vault/workflows.ts');
          const url = new URL(req.url);
          const parts = url.pathname.split('/');
          const id = parts[parts.length - 2];
          if (!id) return error('ID required');
          ...
          for (const [key, value] of Object.entries(body)) {
            await setVariable(id, key, value);
          }
          return json(await getVariables(id));
        } catch (err) { return error(`${err}`); }
      },
    ...
    '/api/workflows/:id/export': {
      GET: async (req: Request) => {
        try {
          const { getWorkflow, getLatestVersion, getVariables } = await import('../vault/workflows.ts');
          const { exportWorkflowYaml } = await import('../workflows/yaml.ts');
          const url = new URL(req.url);
          const parts = url.pathname.split('/');
          const id = parts[parts.length - 2];
          const wf = await getWorkflow(id!);
    ...
    '/api/workflows/executions/:executionId': {
      GET: async (req: Request) => {
        try {
          const { getExecution, getStepResults } = await import('../vault/workflows.ts');
          const url = new URL(req.url);
          ...
          const steps = await getStepResults(executionId);
    ...
    '/api/goals': {
      GET: async (req: Request) => {
        try {
          ...
          const goals = await import('../vault/goals.ts');
          return json(await goals.findGoals({
      ...
      POST: async (req: Request) => {
        try {
          ...
          const goals = await import('../vault/goals.ts');
          const goal = await goals.createGoal(title, level as any, body as any);
    ...
    '/api/goals/roots': {
      GET: async () => {
        try {
          const goals = await import('../vault/goals.ts');
          return json(await goals.getRootGoals());
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/goals/overdue': {
      GET: async () => {
        try {
          const goals = await import('../vault/goals.ts');
          return json(await goals.getOverdueGoals());
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/goals/metrics': {
      GET: async () => {
        try {
          const goals = await import('../vault/goals.ts');
          return json(await goals.getGoalMetrics());
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/goals/check-ins': {
      GET: async (req: Request) => {
        try {
          ...
          const goals = await import('../vault/goals.ts');
          return json(await goals.getRecentCheckIns(type ?? undefined, limit));
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/goals/daily-actions': {
      GET: async () => {
        try {
          const goals = await import('../vault/goals.ts');
          return json(await goals.findGoals({ level: 'daily_action', status: 'active', limit: 20 }));
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/goals/:id': {
      GET: async (req: Request) => {
        try {
          ...
          const id = url.pathname.split('/').pop()!;
          const goals = await import('../vault/goals.ts');
          const goal = await goals.getGoal(id);
      ...
      PATCH: async (req: Request) => {
        try {
          ...
          const body = await req.json() as Record<string, unknown>;
          const goals = await import('../vault/goals.ts');
          const updated = await goals.updateGoal(id, body as any);
      ...
      DELETE: async (req: Request) => {
        try {
          ...
          const id = url.pathname.split('/').pop()!;
          const goals = await import('../vault/goals.ts');
          const deleted = await goals.deleteGoal(id);
    ...
    '/api/goals/:id/tree': {
      GET: async (req: Request) => {
        try {
          ...
          const id = parts[parts.length - 2]!;
          const goals = await import('../vault/goals.ts');
          return json(await goals.getGoalTree(id));
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/goals/:id/children': {
      GET: async (req: Request) => {
        try {
          ...
          const id = parts[parts.length - 2]!;
          const goals = await import('../vault/goals.ts');
          return json(await goals.getGoalChildren(id));
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/goals/:id/score': {
      POST: async (req: Request) => {
        try {
          ...
          const goals = await import('../vault/goals.ts');
          const updated = await goals.updateGoalScore(id, body.score, body.reason, body.source ?? 'user');
          if (!updated) return error('Goal not found', 404);
          return json(updated);
        } catch (err) { return error(`${err}`); }
      },
    ...
    '/api/goals/:id/status': {
      POST: async (req: Request) => {
        try {
          ...
          const goals = await import('../vault/goals.ts');
          const updated = await goals.updateGoalStatus(id, body.status as any);
          if (!updated) return error('Goal not found', 404);
          return json(updated);
        } catch (err) { return error(`${err}`); }
      },
    ...
    '/api/goals/:id/health': {
      POST: async (req: Request) => {
        try {
          ...
          const goals = await import('../vault/goals.ts');
          const updated = await goals.updateGoalHealth(id, body.health as any);
          if (!updated) return error('Goal not found', 404);
          return json(updated);
        } catch (err) { return error(`${err}`); }
      },
    ...
    '/api/goals/:id/progress': {
      GET: async (req: Request) => {
        try {
          ...
          const goals = await import('../vault/goals.ts');
          return json(await goals.getProgressHistory(id, limit));
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/documents': {
      GET: async (req: Request) => {
        try {
          const { findDocuments } = await import('../vault/documents.ts');
          const url = new URL(req.url);
          const format = url.searchParams.get('format') || undefined;
          const tag = url.searchParams.get('tag') || undefined;
          const search = url.searchParams.get('search') || undefined;
          const query = (format || tag || search) ? { format: format as any, tag, search } : undefined;
          return json(await findDocuments(query));
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/documents/:id': {
      GET: async (req: Request) => {
        try {
          const { getDocument } = await import('../vault/documents.ts');
          const url = new URL(req.url);
          const parts = url.pathname.split('/');
      ...
      DELETE: async (req: Request) => {
        try {
          const { deleteDocument } = await import('../vault/documents.ts');
          const url = new URL(req.url);
          const parts = url.pathname.split('/');
          const id = parts[parts.length - 1]!;
    ...
    '/api/sidecars': {
      GET: async () => {
        try {
          ...
          return json(ctx.sidecarManager.getConnectedSidecars());
        } catch (err) { return error(`${err}`); }
      },
    },
    ...
    '/api/sidecars/:id': {
      GET: async (req: Request) => {
        try {
          ...
          const url = new URL(req.url);
          const id = url.pathname.split('/').pop()!;
          const sidecars = await import('../vault/sidecars.ts');
          const sidecar = await sidecars.getSidecar(id);
      ...
      DELETE: async (req: Request) => {
        try {
          ...
          const id = url.pathname.split('/').pop()!;
          const sidecars = await import('../vault/sidecars.ts');
          const revoked = await sidecars.deleteSidecar(id);
    ...
    '/api/sites/templates': {
      GET: async () => {
        const { TEMPLATES } = await import('../sites/templates.ts');
    ...
    '/api/sites/git/check': {
      GET: async () => {
        const { GitManager } = await import('../sites/git-manager.ts');
        ...
        try {
          ...
          if (project?.githubUrl) {
            ...
            const meta = fs.readFileSync(
              path.join(projectPath, '.jarvis-project.json'), 'utf-8'
            );
            const parsed = JSON.parse(meta);
            if (parsed.github) {
              parsed.github.lastPushedAt = Date.now();
              await Bun.write(path.join(projectPath, '.jarvis-project.json'), JSON.stringify(parsed, null, 2));
            }
          }
