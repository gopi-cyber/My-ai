...
import { SiteAgent } from '../sites/site-agent.ts';
...
export class WebSocketService implements Service {
  ...
  constructor(port: number, agentService: AgentService) {
    ...
    this.agentService.setDelegationCallback(async (specialistName) => {
      if (!this.activeTaskId) return;
      try {
        const updated = await updateCommitmentAssignee(this.activeTaskId, specialistName);
  ...
  /**
   * Helper for mobile API and other synchronous-like chat requests.
   */
  async sendChat(text: string, voice?: boolean): Promise<string> {
    const { stream, onComplete } = await this.agentService.streamMessage(text, 'websocket');
    let fullText = '';
    
    // Convert AsyncIterable to full text if it's a stream
    if (Symbol.asyncIterator in stream) {
      for await (const event of stream as any) {
        if (event.type === 'text') fullText += event.text;
      }
    } else {
      fullText = (stream as any).content || String(stream);
    }

    if (onComplete) {
      onComplete(fullText).catch(console.error);
    }
  ...
  /**
   * Returns a list of recently broadcasted notifications.
   */
  getRecentNotifications(limit: number = 20): any[] {
    // Currently we don't persist notifications in memory explicitly here,
    // but the mobile API expects it. For now, return empty or implement a simple cache.
    return [];
  }
  ...
  private async handleChat(msg: WSMessage, ws?: ServerWebSocket<unknown>): Promise<WSMessage | void> {
    ...
    // Use SiteAgent for autonomous site builder tasks
    if (channel === 'site_builder' && projectId && this.siteBuilderService) {
      try {
        const siteAgent = new SiteAgent(this.siteBuilderService, this.agentService.getOrchestrator());
        const { stream, onComplete } = await siteAgent.buildProject(projectId, text);

        const fullText = await this.streamRelay.relayStream(stream, requestId);
        
        // Persist assistant response
        const conversation = await getOrCreateConversation(channel, projectId);
        await addMessage(conversation.id, { role: 'assistant', content: fullText });
        ...
        if (projectPath) {
          ...
          const commit = await this.siteBuilderService.gitManager.autoCommit(projectPath, `[SiteAgent] ${commitMsg}`);
          if (commit) {
        ...
        onComplete(fullText).catch(err => console.error('[WSService] SiteAgent complete error:', err));
        return undefined;
      } catch (err) {
        console.error('[WSService] SiteAgent error:', err);
        return {
          type: 'error',
          payload: { message: err instanceof Error ? err.message : 'SiteAgent failed' },
          id: requestId,
          timestamp: Date.now(),
        };
      }
...
If the user wants to create a new project, tell them to use the Site Builder page (Sites tab in the sidebar) to create one first.`;
    ...
    if (!isTrivial) {
      try {
        ...
        taskCommitment = await createCommitment(taskLabel, {
          assigned_to: 'jarvis',
          created_from: 'user',
        });
        await updateCommitmentStatus(taskCommitment.id, 'active');
    ...
    try {
      const conversation = await getOrCreateConversation(channel);
      await recordUserProfileTurn(text);
      await addMessage(conversation.id, { role: 'user', content: text });
      ...
      const followupPrompt = await maybeCreateUserProfileFollowupPrompt();
      if (followupPrompt) {
        this.broadcastAssistantMessage(followupPrompt);
        await addMessage(conversation.id, { role: 'assistant', content: followupPrompt });
      }
      ...
      if (taskCommitment) {
        try {
          ...
          const updated = await updateCommitmentStatus(taskCommitment.id, 'completed', resultSummary);
          if (updated) this.broadcastTaskUpdate(updated, 'updated');
        } catch (err) {
      ...
      if (taskCommitment) {
        try {
          ...
          const updated = await updateCommitmentStatus(taskCommitment.id, 'failed', reason);
          if (updated) this.broadcastTaskUpdate(updated, 'updated');
        } catch (err) {
