 ...
 * AETHER Daemon
 *
 * Main entry point for the AETHER daemon process.
...
█    █   ███    █▄   ███    ███    ███   ███    █▄   ███    ███ 
 ▀▄▄▀    ███    ███  ███    ███    ███   ███    ███  ███    ███ 
     ...
     A E T H E R  ·  Autonomous Execution & Thought Engine
  `);
...
export async function startDaemon(userConfig?: Partial<DaemonConfig>): Promise<void> {
  ...
  try {
    ...
    await mergeLLMSettingsIntoConfig(jarvisConfig);
    ...
    // --- Pre-registration of secondary services ---
    ...
    if (jarvisConfig.awareness?.enabled !== false && !config.noLocalTools) {
      try {
        ...
        awarenessService = new AwarenessService(
          ...
          (event) => {
            const { classifyEvent } = require('./events.ts'); // late require if needed, but it's likely available
            ...
            if (classified.priority === 'critical' || classified.priority === 'high') {
              reactor.react(classified).catch(err => console.error('[Daemon] Awareness reaction error:', err));
            } else {
            ...
            if (event.type === 'suggestion_ready') {
              ...
              const text = `**${title}**\n${body}`;
              if (wsService.getServer().getClientCount() > 0) {
        ...
        registry.register(awarenessService);
        apiContext.awarenessService = awarenessService;
      } catch (err) {
        console.error('[Daemon] Awareness service failed to register:', err);
      }
    }
    ...
    if (jarvisConfig.sites?.enabled !== false) {
      try {
        ...
        const sitesConfig = jarvisConfig.sites ?? { enabled: true, projects_dir: '~/.jarvis/projects' };
        const sbs = new SiteBuilderService(sitesConfig as any);
        apiContext.siteBuilderService = sbs;
        registry.register(sbs);
      } catch (err) {
        console.error('[Daemon] Site builder failed to register:', err);
      }
    }
    ...
    if (jarvisConfig.workflows?.enabled !== false) {
      try {
        ...
        const wf = new WorkflowEngine(
          nodeRegistry,
          orchestrator.getToolRegistry() ?? new (await import('../actions/tools/registry.ts')).ToolRegistry(),
          agentService.getLLMManager()
        );
        apiContext.workflowEngine = wf;
        apiContext.nodeRegistry = nodeRegistry;
        registry.register(wf);
      } catch (err) {
        console.error('[Daemon] Workflow engine failed to register:', err);
      }
    }
    ...
    if (jarvisConfig.goals?.enabled !== false) {
      try {
        ...
        const goalsConfig = jarvisConfig.goals ?? { enabled: true };
        const gs = new GoalService(goalsConfig as any);
        apiContext.goalService = gs;
        registry.register(gs);
      } catch (err) {
        console.error('[Daemon] Goal service failed to register:', err);
      }
    }
    ...
    // 10d. Initialize Media Engine for generative AI (vision analysis)
    const { getMediaEngine } = await import('../media/engine.ts');
    const mediaEngine = getMediaEngine();
    mediaEngine.setLLM(agentService.getLLMManager());
    console.log('[Daemon] Media Engine initialized (cognitive image/video generation)');
    ...
    // 10a. Finalize secondary service wiring (after startAll)
    
    // Wire Awareness Service
    if (awarenessService) {
      ...
      sidecarManager.onEvent((sidecarId, event) => {
        if (['screen_capture', 'context_changed', 'idle_detected'].includes(event.event_type)) {
          (awarenessService as any).handleSidecarEvent(sidecarId, event).catch(() => {});
        }
      });
      console.log('[Daemon] Awareness service wired to sidecar events');
    }

    // Wire Site Builder Service
    const sbs = apiContext.siteBuilderService;
    if (sbs) {
      wsService.getServer().setSiteProxy((sbs as any).proxy);
      const { createSiteBuilderTools } = await import('../sites/builder-tools.ts');
      const builderTools = createSiteBuilderTools((sbs as any).projectManager, (sbs as any).gitManager, (sbs as any).githubManager);
      ...
      wsService.setSiteBuilderService(sbs as any);
    }

    // Wire Workflow Engine
    const wf = apiContext.workflowEngine;
    if (wf) {
      ...
      (wf as any).setEventCallback((event: any) => wsService.broadcastWorkflowEvent(event));
      const triggerManager = new TriggerManager(wf as any);
      await triggerManager.start();

      const nlBuilder = new NLWorkflowBuilder((apiContext as any).nodeRegistry, agentService.getLLMManager());
      const autoSuggest = new WorkflowAutoSuggest((apiContext as any).nodeRegistry, agentService.getLLMManager());

      const manageWorkflowTool = createManageWorkflowTool({ workflowEngine: wf as any, nlBuilder, triggerManager });
      ...
      console.log('[Daemon] Workflow engine fully wired');
    }

    // Wire Goal Service
    const gs = apiContext.goalService;
    if (gs) {
      ...
      (gs as any).setEventCallback((event: any) => wsService.broadcastGoalEvent(event));
      
      const llm = agentService.getLLMManager();
      const style = jarvisConfig.goals?.accountability_style ?? 'drill_sergeant';
      const goalRhythm = new DailyRhythm(llm, style);
      goalRhythm.setEventCallback((event) => wsService.broadcastGoalEvent(event));
      (gs as any).setRhythm(goalRhythm);
      (gs as any).setChatCallback((text: string) => wsService.broadcastHeartbeat(text));

      const goalNlBuilder = new NLGoalBuilder(llm);
      const goalEstimator = new GoalEstimator(llm);
      const goalAccountability = new AccountabilityEngine(llm, style, jarvisConfig.goals?.escalation_weeks ?? { pressure: 1, root_cause: 3, suggest_kill: 4 });
      
      const manageGoalsTool = createManageGoalsTool({
        goalService: gs as any,
      ...
      console.log('[Daemon] Goal service fully wired');
    }
    ...
    heartbeatTimer = setInterval(async () => {
      ...
      try {
        ...
        const commitmentEvents = await checkCommitments();
        for (const evt of commitmentEvents) {
          if (evt.priority === 'critical' || evt.priority === 'high') {
            await reactor.react(evt).catch(err =>
    ...
    console.log(await healthMonitor.formatHealth());
    console.log('');
