...
describe('DailyRhythm', () => {
  ...
  test('runMorningPlan creates check-in', async () => {
    await vault.createGoal('Active Goal', 'task', { status: 'active' });
    ...
    const todayCheckIn = await vault.getTodayCheckIn('morning_plan');
  ...
  test('runEveningReview creates check-in', async () => {
    ...
    await vault.createCheckIn('morning_plan', 'Morning focus', ['g1'], ['Write code']);
  ...
  test('runEveningReview works without morning plan', async () => {
    await vault.createGoal('Goal', 'task', { status: 'active' });
    const result = await rhythm.runEveningReview();
    expect(result.checkIn).toBeTruthy();
  });
});
...
describe('AccountabilityEngine', () => {
  ...
  test('runEscalationCheck returns empty for healthy goals', async () => {
    await vault.createGoal('Healthy', 'task', { status: 'active' });
    const actions = await engine.runEscalationCheck();
    expect(actions.length).toBe(0);
  });

  test('runEscalationCheck detects goals needing escalation', async () => {
    const goal = await vault.createGoal('Behind Goal', 'task', { status: 'active' });
    await vault.updateGoalHealth(goal.id, 'behind');
  ...
  test('generateEscalationMessage returns text', async () => {
    const goal = await vault.createGoal('Failing Goal', 'task', { status: 'active' });
    await vault.updateGoalHealth(goal.id, 'behind');

    const message = await engine.generateEscalationMessage(
      (await vault.getGoal(goal.id))!,
      'pressure',
    );
  ...
  test('generateEscalationMessage returns empty for none stage', async () => {
    const goal = await vault.createGoal('OK Goal', 'task', { status: 'active' });
    const message = await engine.generateEscalationMessage((await vault.getGoal(goal.id))!, 'none');
    expect(message).toBe('');
  });

  test('generateReplanOptions returns options', async () => {
    const goal = await vault.createGoal('Stuck Goal', 'objective', {
      status: 'active',
      description: 'Big project',
      success_criteria: 'Ship it',
    });

    const analysis = await engine.generateReplanOptions((await vault.getGoal(goal.id))!);
