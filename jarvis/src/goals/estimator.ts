...
import type { GoalEstimate, GoalLevel, Goal } from './types.ts';
...
export class GoalEstimator {
  ...
  async estimate(goalId: string): Promise<GoalEstimate | null> {
    ...
    const similarGoals = await this.findSimilarCompleted(goal.title, goal.level, goal.description);
   ...
   */
  async quickEstimate(title: string, level: GoalLevel): Promise<{ hours: number; confidence: number }> {
    const similar = await this.findSimilarCompleted(title, level);
  ...
  private async findSimilarCompleted(title: string, level: GoalLevel, description?: string): Promise<Goal[]> {
    // Find completed goals at the same level
    const completed = await vault.findGoals({ status: 'completed', level, limit: 50 });
  ...
  private calculateHistoricalEstimate(goals: Goal[]): number | null {
