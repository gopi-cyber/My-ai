...
export class DailyRhythm {
  ...
  async runMorningPlan(): Promise<MorningPlanResult> {
    const activeGoals = await vault.findGoals({ status: 'active', limit: 20 });
    const overdueGoals = await vault.getOverdueGoals();
    const yesterdayEvening = await vault.getRecentCheckIns('evening_review', 1);

    const goalSummary = activeGoals.map((g: Goal) =>
    ...
    const overdueSummary = overdueGoals.length > 0
      ? `\n\nOVERDUE GOALS:\n${overdueGoals.map((g: Goal) => `- ${g.title} (due ${new Date(g.deadline!).toLocaleDateString()})`).join('\n')}`
      : '';
    ...
    try {
      ...
      const goalsReviewed = activeGoals.map((g: Goal) => g.id);

      const checkIn = await vault.createCheckIn(
        'morning_plan',
  ...
  async runEveningReview(): Promise<EveningReviewResult> {
    const activeGoals = await vault.findGoals({ status: 'active', limit: 20 });
    const morningCheckIn = await vault.getTodayCheckIn('morning_plan');
    ...
    try {
      ...
      for (const update of scoreUpdates) {
        await vault.updateGoalScore(update.goalId, update.newScore, update.reason, 'daily_review');
      }
