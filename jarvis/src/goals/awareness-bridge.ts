 ...
 */
export async function matchAwarenessToGoals(
  eventData: Record<string, unknown>,
): Promise<AwarenessGoalMatch[]> {
 ...
 */
export async function logAutoDetectedProgress(
  ...
  for (const match of matches) {
    ...
    const recentProgress = await vault.getProgressHistory(match.goalId, 5);
    ...
    const goal = await vault.getGoal(match.goalId);
    if (!goal) continue;

    // Log progress entry (no score change, just detection)
    await vault.addProgressEntry(
      match.goalId,
      'auto_detected',
      goal.score,
