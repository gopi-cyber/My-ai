...
export class AccountabilityEngine {
   ...
   */
  async runEscalationCheck(): Promise<EscalationAction[]> {
  ...
  async generateEscalationMessage(goal: Goal, stage: EscalationStage): Promise<string> {
    ...
    const history = await vault.getProgressHistory(goal.id, 10);
  ...
  async generateReplanOptions(goal: Goal): Promise<ReplanAnalysis> {
    const children = await vault.getGoalChildren(goal.id);
    const history = await vault.getProgressHistory(goal.id, 20);
