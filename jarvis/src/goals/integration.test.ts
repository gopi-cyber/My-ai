...
test('matchAwarenessToGoals returns empty when no active goals', async () => {
  const matches = await matchAwarenessToGoals({
    app_name: 'VS Code',
...
test('matchAwarenessToGoals finds matches for active goals', async () => {
  await createGoal('Learn TypeScript fundamentals', 'task', {
    description: 'Complete TypeScript tutorial covering generics, interfaces, and type guards',
    status: 'active',
  });
  await createGoal('Exercise daily', 'daily_action', {
...
test('matchAwarenessToGoals does not match unrelated events', async () => {
  await createGoal('Learn Python machine learning', 'task', {
...
test('matchAwarenessToGoals ignores non-active goals', async () => {
  await createGoal('Learn TypeScript fundamentals', 'task', {
...
test('matchAwarenessToGoals handles session_ended data', async () => {
  await createGoal('Build web application with React', 'milestone', {
...
test('logAutoDetectedProgress creates progress entries', async () => {
  const goal = await createGoal('Learn TypeScript', 'task', {
  ...
  await logAutoDetectedProgress(matches, 'context_changed');

  // Check progress was logged
  const { getProgressHistory } = require('../vault/goals.ts');
  const progress = await getProgressHistory(goal.id, 10);
...
test('logAutoDetectedProgress deduplicates within 30 minutes', async () => {
  const goal = await createGoal('Learn TypeScript', 'task', {
    description: 'Complete TypeScript tutorial',
    status: 'active',
  });
...
test('extractGoalCompletion creates entity with facts', async () => {
  const goal = await createGoal('Build REST API', 'milestone', {
  ...
  await extractGoalCompletion(completedGoal);

  // Verify entity was created
  const entities = await findEntities({ name: 'Build REST API', type: 'concept' });
  expect(entities.length).toBe(1);
  expect(entities[0]!.source).toBe('goal_completion');

  // Verify facts were stored
  const facts = await findFacts({ subject_id: entities[0]!.id });
...
test('extractGoalCompletion handles failed goals', async () => {
  await extractGoalCompletion({
    id: 'test-failed',
    title: 'Learn Rust',
    level: 'objective',
    score: 0.2,
  ...
  const entities = await findEntities({ name: 'Learn Rust', type: 'concept' });
  expect(entities.length).toBe(1);
...
test('getActiveGoalsSummary returns formatted goal list', async () => {
  await createGoal('Get fit', 'objective', { status: 'active' });
  await createGoal('Run daily', 'key_result', { status: 'active' });
  await createGoal('Complete project', 'milestone', { status: 'active' });

  const summary = await getActiveGoalsSummary();
...
test('getActiveGoalsSummary returns empty when no active goals', async () => {
  await createGoal('Done goal', 'task', { status: 'completed' });

  const summary = await getActiveGoalsSummary();
  expect(summary).toBe('');
});

test('getActiveGoalsSummary sorts by level hierarchy', async () => {
  await createGoal('Daily run', 'daily_action', { status: 'active' });
  await createGoal('Get fit', 'objective', { status: 'active' });
  await createGoal('Lose 10 lbs', 'key_result', { status: 'active' });
