...
describe('NLGoalBuilder', () => {
  ...
  test('createFromProposal creates goal hierarchy', async () => {
    ...
    const goals = await builder.createFromProposal(proposal);
  ...
  test('createFromProposal with parent_id', async () => {
    const parent = await vault.createGoal('Parent Objective', 'objective');
    const proposal = await builder.parseGoal('Sub-objective');
    const goals = await builder.createFromProposal(proposal, parent.id);

    expect(goals[0]!.parent_id).toBe(parent.id);
  });

  test('decompose returns proposal for children', async () => {
    const goal = await vault.createGoal('Root Objective', 'objective', {
  ...
  test('decompose returns null for daily_action (no next level)', async () => {
    const goal = await vault.createGoal('Daily task', 'daily_action');
    const proposal = await builder.decompose(goal.id);
    expect(proposal).toBeNull();
  });

  test('chat returns text reply', async () => {
    const goal = await vault.createGoal('Chat goal', 'objective');
...
describe('GoalEstimator', () => {
  ...
  test('quickEstimate returns heuristic for no history', async () => {
    const est = await estimator.quickEstimate('Build auth', 'task');
    expect(est.hours).toBe(4);
    expect(est.confidence).toBe(0.2);
  });

  test('quickEstimate uses history when available', async () => {
    // Create some completed goals with actual hours
    const g1 = await vault.createGoal('Build login', 'task', { status: 'active' });
    await vault.updateGoalActualHours(g1.id, 6);
    await vault.updateGoalStatus(g1.id, 'completed');

    const g2 = await vault.createGoal('Build signup', 'task', { status: 'active' });
    await vault.updateGoalActualHours(g2.id, 8);
    await vault.updateGoalStatus(g2.id, 'completed');

    const est = await estimator.quickEstimate('Build auth flow', 'task');
  ...
  test('estimate returns LLM-based estimate when no history', async () => {
    const goal = await vault.createGoal('New feature', 'key_result', {
  ...
  test('estimate blends history and LLM', async () => {
    ...
    const g1 = await vault.createGoal('Build feature alpha', 'key_result', {
      status: 'active',
      description: 'Build something',
    });
    await vault.updateGoalActualHours(g1.id, 30);
    await vault.updateGoalStatus(g1.id, 'completed');

    const g2 = await vault.createGoal('Build feature beta', 'key_result', {
      status: 'active',
      description: 'Build another thing',
    });
    await vault.updateGoalActualHours(g2.id, 50);
    await vault.updateGoalStatus(g2.id, 'completed');

    // Create goal to estimate
    const goal = await vault.createGoal('Build feature gamma', 'key_result', {
