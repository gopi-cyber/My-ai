...
export class NLGoalBuilder {
  ...
  async parseGoal(text: string): Promise<GoalProposal> {
    const existingGoals = (await vault.getRootGoals()).slice(0, 10);
  ...
  async chat(
    ...
    const goal = await vault.getGoal(goalId);
    const tree = goal ? await vault.getGoalTree(goalId) : [];
   ...
   */
  async createFromProposal(proposal: GoalProposal, parentId?: string): Promise<Goal[]> {
    const created: Goal[] = [];

    const objective = await vault.createGoal(proposal.objective.title, 'objective', {
      parent_id: parentId,
    ...
    for (let i = 0; i < proposal.key_results.length; i++) {
      ...
      const keyResult = await vault.createGoal(kr.title, 'key_result', {
      ...
      if (proposal.milestones) {
        for (const ms of proposal.milestones) {
          if (ms.key_result_index === i) {
            const milestone = await vault.createGoal(ms.title, 'milestone', {
