...
export class GoalService implements Service {
  ...
  async createGoal(title: string, level: GoalLevel, opts?: Parameters<typeof vault.createGoal>[2]): Promise<Goal> {
    const goal = await vault.createGoal(title, level, opts);
    this.emit({
      type: 'goal_created',
      goalId: goal.id,
  ...
  async updateGoal(id: string, updates: Parameters<typeof vault.updateGoal>[1]): Promise<Goal | null> {
    const goal = await vault.updateGoal(id, updates);
    if (goal) {
      this.emit({
        type: 'goal_updated',
        goalId: id,
  ...
  async scoreGoal(id: string, score: number, reason: string, source = 'user'): Promise<Goal | null> {
    const goal = await vault.updateGoalScore(id, score, reason, source);
    if (goal) {
      this.emit({
        type: 'goal_scored',
        goalId: id,
  ...
  async updateStatus(id: string, status: GoalStatus): Promise<Goal | null> {
    const goal = await vault.updateGoalStatus(id, status);
    if (!goal) return null;
  ...
  async updateHealth(id: string, health: GoalHealth): Promise<Goal | null> {
    const goal = await vault.updateGoalHealth(id, health);
    if (goal) {
      this.emit({
  ...
  async deleteGoal(id: string): Promise<boolean> {
    const result = await vault.deleteGoal(id);
    if (result) {
      this.emit({
        type: 'goal_deleted',
        goalId: id,
  ...
  private async checkDailyRhythm(): Promise<void> {
    ...
    if (hour >= morningWindow.start && hour < morningWindow.end) {
      const existing = await vault.getTodayCheckIn('morning_plan');
      if (!existing) {
    ...
    if (hour >= eveningWindow.start && hour < eveningWindow.end) {
      const existing = await vault.getTodayCheckIn('evening_review');
      if (!existing) {
  ...
  private async checkAccountability(): Promise<void> {
    const needingEscalation = await vault.getGoalsNeedingEscalation();
    ...
    for (const goal of needingEscalation) {
      if (goal.escalation_stage === 'none') {
        ...
        if (weeksBehind >= escalationWeeks.pressure) {
          await vault.updateGoalEscalation(goal.id, 'pressure');
          this.emit({
            type: 'goal_escalated',
            goalId: goal.id,
        ...
        if (weeksSinceEscalation >= escalationWeeks.root_cause) {
          await vault.updateGoalEscalation(goal.id, 'root_cause');
          this.emit({
            type: 'goal_escalated',
            goalId: goal.id,
        ...
        if (weeksSinceEscalation >= escalationWeeks.suggest_kill) {
          await vault.updateGoalEscalation(goal.id, 'suggest_kill');
          this.emit({
            type: 'goal_escalated',
            goalId: goal.id,
  ...
  private async recalculateAllHealth(): Promise<void> {
    const activeGoals = await vault.findGoals({ status: 'active' });
    let changed = 0;
