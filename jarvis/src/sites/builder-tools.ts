            ...
            (async () => {
              ...
              const streamOutput = async (stream: ReadableStream, isError: boolean) => {
                ...
                while (true) {
                  ...
                  if (isError) stderrContent += text;
                  else stdoutContent += text;

                  // Stream to dashboard if callback exists
                  if (context?.onProgress && text.trim()) {
                    context.onProgress({
                      type: 'text',
                      agentName: 'System', 
                      agentId: 'system',
              ...
              let output = '';
              if (stdoutContent.trim()) output += stdoutContent.trim();
              if (stderrContent.trim()) output += (output ? '\n' : '') + stderrContent.trim();
