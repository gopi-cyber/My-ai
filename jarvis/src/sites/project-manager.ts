...
export class ProjectManager {
  ...
  /**
   * Helper to spawn processes in a platform-aware way.
   * On Windows, shell-integrated commands like 'bunx' or 'make' often need to be run via 'cmd /c'.
   */
  private spawnPlatform(command: string, args: string[], options: any) {
    const isWindows = process.platform === 'win32';
    const cmd = isWindows ? 'cmd' : command;
    const finalArgs = isWindows ? ['/c', command, ...args] : args;

    return Bun.spawn([cmd, ...finalArgs], options);
  }
  ...
  async createProject(name: string, templateId: string, gitAuthor?: { name: string; email: string; global: boolean }): Promise<Project> {
    ...
    if (template.command === 'scaffold') {
      ...
      const proc = this.spawnPlatform(template.command, args, {
    ...
    const installProc = this.spawnPlatform('make', ['install'], {
      cwd: projectPath,
      stdout: 'pipe',
      stderr: 'pipe',
    });
