  ...
  return {
    ...
    execute: async (params) => {
      ...
      switch (action) {
        case 'create': {
          ...
          if (text) {
            ...
            try {
              ...
              const goals = await deps.nlBuilder.createFromProposal(
                proposal,
          ...
          if (title) {
            ...
            const goal = await deps.goalService.createGoal(title, level as any, {
        ...
        case 'list': {
          const goals = await vault.findGoals({
        ...
        case 'get': {
          ...
          const children = await vault.getGoalChildren(goalId);
          const progress = await vault.getProgressHistory(goalId, 5);
        ...
        case 'score': {
          ...
          const goal = await deps.goalService.scoreGoal(goalId, score, reason);
          if (!goal) return `Goal "${goalId}" not found.`;
        ...
        case 'update_status': {
          ...
          const goal = await deps.goalService.updateStatus(goalId, status as any);
          if (!goal) return `Goal "${goalId}" not found.`;
        ...
        case 'update': {
          ...
          const goal = await deps.goalService.updateGoal(goalId, updates as any);
          if (!goal) return `Goal "${goalId}" not found.`;
        ...
        case 'decompose': {
          ...
          try {
            ...
            const goals = await deps.nlBuilder.createFromProposal(proposal, goalId);
            const summary = goals.map(g =>
              `${'  '.repeat(levelDepth(g.level))}${g.level}: ${g.title}`
            ).join('\n');
        ...
        case 'metrics': {
          const m = await deps.goalService.getMetrics();
          return `**Goal Metrics**\n` +
        ...
        case 'delete': {
          const goalId = params.goal_id as string;
          if (!goalId) return 'Error: "goal_id" is required.';
          const deleted = await deps.goalService.deleteGoal(goalId);
        ...
        case 'tree': {
          ...
          const tree = await vault.getGoalTree(goalId);
        ...
        case 'escalations': {
          const actions = await deps.accountability.runEscalationCheck();
