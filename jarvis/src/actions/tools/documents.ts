  ...
  execute: async (params) => {
    ...
    switch (action) {
      ...
      case 'list': {
        ...
        const docs = await findDocuments(Object.keys(query).length > 0 ? query : undefined);
      ...
      case 'update': {
        ...
        const updated = await updateDocument(params.id as string, updates);
      ...
      case 'append': {
        ...
        const existing = await getDocument(params.id as string);
        if (!existing) return `Document not found: ${params.id}`;
        const newBody = existing.body + (existing.body ? '\n\n' : '') + (params.body as string);
        const updated = await updateDocument(params.id as string, { body: newBody });
      ...
      case 'delete': {
        ...
        const deleted = await deleteDocument(params.id as string);
