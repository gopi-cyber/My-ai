 ...
 */
export async function resolveDefaultSidecar(
  requiredCapability: SidecarCapability,
): Promise<{ sidecar: SidecarInfo } | { error: string }> {
  if (!sidecarManager) {
    return { error: 'Error: Sidecar system not initialized.' };
  }

  const sidecars = await sidecarManager.listSidecars();
...
export async function routeToSidecarOrDefault(
  ...
  const resolved = await resolveDefaultSidecar(requiredCapability);
