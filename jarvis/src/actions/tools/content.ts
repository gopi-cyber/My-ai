  ...
  execute: async (params) => {
    ...
    switch (action) {
      case 'list': {
        ...
        const items = await findContent(query);
      ...
      case 'get': {
        ...
        const item = await getContent(params.id as string);
      ...
      case 'create': {
        ...
        const item = await createContent(params.title as string, {
      ...
      case 'update': {
        ...
        const updated = await updateContent(params.id as string, updates);
      ...
      case 'set_body': {
        ...
        const updated = await updateContent(params.id as string, { body: params.body as string });
        if (!updated) return `Content item not found: ${params.id}`;
      ...
      case 'append_body': {
        ...
        const existing = await getContent(params.id as string);
        if (!existing) return `Content item not found: ${params.id}`;
        const newBody = existing.body + (existing.body ? '\n\n' : '') + (params.body as string);
        const updated = await updateContent(params.id as string, { body: newBody });
      ...
      case 'advance': {
        ...
        const advanced = await advanceStage(params.id as string);
      ...
      case 'regress': {
        ...
        const regressed = await regressStage(params.id as string);
      ...
      case 'add_note': {
        ...
        const stageNote = await addStageNote(
          params.id as string,
      ...
      case 'get_notes': {
        ...
        const notes = await getStageNotes(
          params.id as string,
