...
describe('resolveDefaultSidecar', () => {
  ...
  test('returns error when sidecar manager is not initialized', async () => {
    setSidecarManagerRef(null as any);
    const result = await resolveDefaultSidecar('desktop');
  ...
  test('returns error when no sidecars are enrolled', async () => {
  ...
  test('returns error when sidecars enrolled but none connected', async () => {
    setSidecarManagerRef(createMockManager([
  ...
  test('returns error when connected but no matching capability', async () => {
    setSidecarManagerRef(createMockManager([
  ...
  test('returns sidecar when exactly one connected with matching capability', async () => {
  ...
  test('returns error when multiple connected sidecars have matching capability', async () => {
    setSidecarManagerRef(createMockManager([
  ...
  test('excludes sidecars with unavailable capability from auto-resolution', async () => {
    setSidecarManagerRef(createMockManager([
      makeSidecar({
  ...
  test('resolves correct sidecar when one has capability unavailable and another does not', async () => {
    setSidecarManagerRef(createMockManager([
      makeSidecar({
  ...
  test('resolves screenshot capability independently from desktop', async () => {
    setSidecarManagerRef(createMockManager([
      makeSidecar({ id: 's1', name: 'my-pc', connected: true, capabilities: ['screenshot'] }),
    ]));
    const desktopResult = await resolveDefaultSidecar('desktop');
    expect('error' in desktopResult).toBe(true);

    const screenshotResult = await resolveDefaultSidecar('screenshot');
  ...
  test('ignores offline sidecars even if they have the capability', async () => {
    setSidecarManagerRef(createMockManager([
