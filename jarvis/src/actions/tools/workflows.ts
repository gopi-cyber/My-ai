      ...
      switch (action) {
        case 'create': {
          ...
          try {
            ...
            await vault.createVersion(workflow.id, definition, 'Created via chat');
        ...
        case 'list': {
          const workflows = await vault.findWorkflows();
        ...
        case 'enable': {
          ...
          try {
            await vault.updateWorkflow(id, { enabled: true });
            const version = await vault.getLatestVersion(id);
            if (version) {
        ...
        case 'disable': {
          ...
          try {
            await vault.updateWorkflow(id, { enabled: false });
            deps.triggerManager.unregisterWorkflow(id);
