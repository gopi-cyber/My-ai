  ...
  execute: async (params) => {
    ...
    switch (action) {
      case 'list': {
        ...
        const items = await findCommitments(query);
      ...
      case 'get': {
        ...
        const item = await getCommitment(params.id as string);
      ...
      case 'create': {
        ...
        const item = await createCommitment(params.what as string, {
          when_due: whenDue,
      ...
      case 'set_due': {
        ...
        const updated = await updateCommitmentDue(params.id as string, whenDue);
        if (!updated) return `Commitment not found: ${params.id}`;
