...
export class WorkflowEngine implements Service {
  ...
  async start(): Promise<void> {
    ...
    // Register as sub-workflow executor
    vault.registerSubWorkflowExecutor(async (workflowId, inputs) => {
      return this.execute(workflowId, 'subworkflow', inputs);
    });
  }
  ...
  async execute(
    ...
    const workflow = await vault.getWorkflow(workflowId);
    if (!workflow) throw new Error(`Workflow '${workflowId}' not found`);
    if (!workflow.enabled) throw new Error(`Workflow '${workflow.name}' is disabled`);

    const version = await vault.getLatestVersion(workflowId);
    if (!version) throw new Error(`No versions found for workflow '${workflow.name}'`);

    const definition = version.definition;

    // Create execution record
    const execution = await vault.createExecution(workflowId, version.version, triggerType, triggerData);
  ...
  async cancel(executionId: string): Promise<void> {
    ...
    if (controller) {
      controller.abort();
      await vault.updateExecution(executionId, { status: 'cancelled', completed_at: Date.now() });
      this.activeExecutions.delete(executionId);

      const exec = await vault.getExecution(executionId);
      this.emit({
  ...
  private async runExecution(
    ...
    const persistentVars = await vault.getVariables(workflowId);
    const variables = new VariableScope(workflowId, persistentVars);
    ...
    try {
      ...
      await vault.updateExecution(executionId, {
        status: 'completed',
  ...
  private async executeStep(
    ...
    const stepResult = await vault.createStepResult(executionId, nodeId, node.type);
    await vault.updateStepResult(stepResult.id, { status: 'running', started_at: Date.now() });
    ...
    try {
      ...
      await vault.updateStepResult(stepResult.id, {
        status: 'completed',
