...
export class LLMManager {
  ...
  private static readonly AUTO_FAILOVER_THRESHOLD = 3;
    ...
    for (const providerName of this.getProviderSequence(overridePrimary)) {
      ...
      for (let attempt = 1; attempt <= LLMManager.MAX_RETRIES_PER_PROVIDER; attempt++) {
        try {
          ...
          // Reset failure count if the primary succeeds
          if (providerName === this.primaryProvider && !overridePrimary) {
      ...
      // If we got here, all retries for this provider failed.
      // If this was the primary, increment failure count and potentially failover.
      if (providerName === this.primaryProvider && !overridePrimary) {
        this.primaryFailureCount++;
        if (this.primaryFailureCount >= LLMManager.AUTO_FAILOVER_THRESHOLD && this.fallbackChain.length > 0) {
          const oldPrimary = this.primaryProvider;
          const newPrimary = this.fallbackChain[0]!;
          console.warn(`[LLM] Primary provider ${oldPrimary} failed ${this.primaryFailureCount} times. Promoting ${newPrimary} to primary.`);
          
          this.primaryProvider = newPrimary;
          this.fallbackChain = [...this.fallbackChain.slice(1), oldPrimary];
          this.primaryFailureCount = 0;
        }
      }
...
/**
 * Returns a wrapper for the primary LLM provider
 */
export function getLLM() {
  const manager = getLLMManager();
  return {
    chat: (messages: LLMMessage[], options?: LLMOptions) => manager.chat(messages, options),
    stream: (messages: LLMMessage[], options?: LLMOptions) => manager.stream(messages, options),
  };
}
