...
export class BehaviorAnalytics {
  ...
  async generateDailyReport(date?: string): Promise<DailyReport> {
    ...
    const captures = await getCapturesInRange(dayStart, dayEnd);
    const appBreakdown = await getAppUsageStats(dayStart, dayEnd);
    const suggestionStats = await getSuggestionStats(dayStart, dayEnd);
   ...
   */
  async getAppUsage(startTime: number, endTime: number): Promise<AppUsageStat[]> {
    return await getAppUsageStats(startTime, endTime);
  }

  /**
   * Get recent session history.
   */
  async getSessionHistory(limit: number = 20): Promise<SessionSummary[]> {
   ...
   */
  async getLiveContext(tracker: ContextTracker, isRunning: boolean): Promise<LiveContext> {
    ...
    try {
      const stats = await getAppUsageStats(tenMinAgo, Date.now());
    ...
    try {
      ...
      capturesLastHour = await getCaptureCountSince(oneHourAgo);

      const todayStart = new Date();
      todayStart.setHours(0, 0, 0, 0);
      suggestionsToday = await getSuggestionCountSince(todayStart.getTime());
    } catch { /* ignore */ }
  ...
  async generateWeeklyReport(weekStart?: string): Promise<WeeklyReport> {
    ...
    const recentSessionsLimit = await getRecentSessions(100);
    ...
    for (let d = 0; d < 7; d++) {
      ...
      const sessionCount = recentSessionsLimit.filter(s => s.started_at >= dayStart && s.started_at <= dayEnd).length;
    ...
    const topApps = await getAppUsageStats(weekStartMs, weekEndMs);
   ...
   */
  async getBehavioralInsights(days: number = 7): Promise<BehavioralInsight[]> {
    ...
    const currentCaptures = await getCapturesInRange(currentStart, now);
    const prevCaptures = await getCapturesInRange(prevStart, currentStart);
    ...
    const currentApps = await getAppUsageStats(currentStart, now);
