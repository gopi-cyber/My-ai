...
export type ScheduleDeps = {
  ...
  getUpcomingCommitments?: () => Promise<Array<{ what: string; when_due: number | null; priority: string }>>;
};
...
export class SuggestionEngine {
  ...
  async evaluate(
    ...
    const candidates = [
      ...
      await this.checkKnowledge(context, events),
      null, // index 5: schedule
      await this.checkBreak(context),
    ...
    if (candidates.every(c => c === null)) {
      const scheduleSuggestion = await this.checkSchedule(context);
      (candidates as any)[5] = scheduleSuggestion;
    }
   ...
   */
  private async checkKnowledge(context: ScreenContext, events: AwarenessEvent[]): Promise<Suggestion | null> {
    if (!events.some(e => e.type === 'context_changed')) return null;
    ...
    try {
      ...
      for (const word of words) {
        const entities = await searchEntitiesByName(word);
        ...
        for (const entity of relevant.slice(0, 3)) {
          ...
          const factSummary = facts
            .slice(0, 3)
            .map((f: any) => `${f.predicate}: ${f.object}`)
            .join('; ');
  ...
  private async checkSchedule(context: ScreenContext): Promise<Suggestion | null> {
    ...
    try {
      ...
      if (this.scheduleDeps.getUpcomingCommitments) {
        const commitments = await this.scheduleDeps.getUpcomingCommitments();
   ...
   */
  private async checkBreak(context: ScreenContext): Promise<Suggestion | null> {
    try {
      const ninetyMinAgo = Date.now() - 90 * 60 * 1000;

      // Don't suggest a break if we already suggested one recently
      const recentBreakSuggestions = await getSuggestionCountSince(ninetyMinAgo);
      ...
      const captureCount = await getCaptureCountSince(ninetyMinAgo);
