...
export class ContextTracker {
   ...
   */
  async processCapture(captureId: string, ocrText: string, rawWindowTitle?: string): Promise<{
    ...
    if (isAppChange || isIdleReturn || !this.currentSessionId) {
      ...
      // Start new session
      const session = await createSession({ startedAt: now, apps: [appName] });
    ...
    if (this.currentSessionId && appName) {
      ...
      await incrementSessionCaptureCount(this.currentSessionId);
    }
