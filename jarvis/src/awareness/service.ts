...
export class AwarenessService implements Service {
  ...
  constructor(
    ...
    this.suggestionEngine = new SuggestionEngine(cfg.suggestion_rate_limit_ms, {
      ...
      getUpcomingCommitments: async () => {
        const commitments = await getUpcoming(10);
  ...
  async stop(): Promise<void> {
    ...
    // End current session
    await this.contextTracker.endCurrentSession();
  ...
  async getLiveContext(): Promise<LiveContext> {
    return await this.analytics.getLiveContext(this.contextTracker, this._status === 'running');
  }
  ...
  async getRecentSuggestionsList(limit?: number, type?: SuggestionType): Promise<SuggestionRow[]> {
    return await getRecentSuggestions(limit, type);
  }

  async dismissSuggestion(id: string): Promise<void> {
    await markSuggestionDismissed(id);
  }

  async actOnSuggestion(id: string): Promise<void> {
    await markSuggestionActedOn(id);
  }
  ...
  async getSessionHistory(limit?: number): Promise<SessionSummary[]> {
    return await this.analytics.getSessionHistory(limit);
  }
  ...
  async getBehavioralInsights(days?: number): Promise<BehavioralInsight[]> {
    return await this.analytics.getBehavioralInsights(days);
  }
  ...
  private async cleanupRetention(): Promise<void> {
    try {
      ...
      let keyDeleted = 0;
      try {
        fullDeleted = await deleteCapturesBefore(fullCutoff, 'full');
        keyDeleted = await deleteCapturesBefore(keyMomentCutoff, 'key_moment');
  ...
  private async processCaptureEvent(data: {
    ...
    try {
      ...
      const { context, events } = await this.contextTracker.processCapture(
        data.captureId,
        ocrText,
        windowTitle
      );
      ...
      // 3. Entity linking
      await this.contextGraph.linkCaptureToEntities(context);
      ...
      if (events.some(e => keyMomentEventTypes.includes(e.type))) {
        try { await updateCaptureRetention(data.captureId, 'key_moment'); } catch { /* best-effort */ }
      }

      // 5. Store as observation
      try {
        await createObservation('screen_capture', {
      ...
      if (suggestion) {
        try { await markSuggestionDelivered(suggestion.id, 'websocket'); } catch { /* ignore */ }
  ...
  private async inferSessionTopic(data: { sessionId: string; apps: string[] }): Promise<void> {
    ...
    try {
      ...
      const captures = await getCapturesForSession(sessionId);
      ...
      await updateSession(sessionId, { topic, summary });
