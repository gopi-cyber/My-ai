...
export default function AwarenessPage() {
  ...
  // Poll status every 5s - only if tab is "live" or "timeline" to save resources
  useEffect(() => {
    ...
    <div className="aw-page">
      ...
      {(status === null || error) && !loading && (
        <div style={{
          ...
          <div>
            <strong>System Link Failure:</strong> {error || "The awareness service could not be reached."}
          </div>
          <div style={{ fontSize: "12px", opacity: 0.8 }}>
            Ensure the AETHER daemon is running with awareness enabled in the configuration.
          </div>
          <button 
      ...
      {usageEstimate ? (
        ...
        >
          ...
          Awareness samples about {usageEstimate?.capturesPerHour ?? 0}/hr and may escalate up to{" "}
          {usageEstimate?.estimatedVisionCallsPerHour ?? 0}/hr for cloud vision. {usageEstimate?.note || ""}
        </div>
      ) : null}
