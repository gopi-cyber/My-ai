...
export default function OfficePage({ agentActivity }: Props) {
  ...
  function getLiveInstances(roleId: string): LiveAgentInfo[] {
  ...
  const allAgents: AgentWithLive[] = AGENT_ROSTER.map((r) => {
    const instances = getLiveInstances(r.roleId);
    return {
      ...r,
      live: instances[0] || null,
      instances, // Add all instances for detailed view
    };
  });
