...
function AgentCard({ agent }: { agent: AgentWithLive }) {
  ...
  let statusLabel = "";
  let statusClass = "ag-status-badge";
  ...
  if (isPrimary) {
    ...
    statusLabel = instanceCount > 1 ? `${instanceCount} Active` : "Active";
  ...
  return (
    <div className={cardClass}>
      <div className="ag-card-body">
        <div className={`ag-avatar ${agent.avatarBg}${isBusy ? " busy-pulse" : ""}`}>{agent.emoji}</div>
