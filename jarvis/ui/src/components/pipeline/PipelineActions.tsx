...
const AETHER_ACTIONS: Record<string, { label: string; prompt: string }[]> = {
  idea: [
...
export function PipelineActions({ contentId, stage, title, onAdvance, onRegress, onDelete, sendMessage }: Props) {
  ...
  const aetherActions = AETHER_ACTIONS[stage] || [];

  const handleAetherAction = (action: { label: string; prompt: string }) => {
  ...
  return (
    <div>
      ...
      {/* AETHER assist buttons */}
      {aetherActions.length > 0 && (
