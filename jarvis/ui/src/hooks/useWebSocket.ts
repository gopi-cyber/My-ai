...
function createSidecarNotice(payload: SidecarEventPayload, timestamp?: number): ChatMessage & { notice?: SystemNotice } {
  ...
  const notice: SystemNotice = {
    ...
    text: reason
      ? `Aether sidecar disconnected: ${reason}. Dashboard features may be delayed until it reconnects.`
      : "Aether sidecar disconnected. Dashboard features may be delayed until it reconnects.",
    level: "warning",
  };
...
export function useWebSocket() {
  ...
  // Clean up stale activity (older than 10 minutes)
  useEffect(() => {
    const interval = setInterval(() => {
      const tenMinutesAgo = Date.now() - 10 * 60 * 1000;
      setAgentActivity((prev) => {
        const filtered = prev.filter((a) => a.timestamp > tenMinutesAgo);
